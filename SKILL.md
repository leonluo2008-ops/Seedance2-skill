...
使用 `--video-ref` 传入本地视频文件，脚本自动上传到 nginx 静态托管，返回公网 HTTPS URL 并设置 `role: "reference_video"`。

### 工作原理

```
本地视频文件
    ↓ docker cp 到容器 /www/video/<md5>_<name>.mp4
    ↓ nginx 静态服务（端口 80）
    ↓ Cloudflare Tunnel 暴露公网
公网 URL: https://img.aistar.work/video/<md5>_<name>.mp4
    ↓ 作为 reference_video 传入 API
Seedance 2.0 运动复刻
```

**背景**：`img.aistar.work` 是本地图床域名，通过 Cloudflare Tunnel（tunnel 名 `n8nmak`）暴露公网。nginx 容器名 `1Panel-openresty-kfra`，图床服务监听 `127.0.0.1:40061`，`/video/` 路径优先于图床代理匹配。

**失败时的操作指引**：
- Docker 不存在 → 安装 Docker 或手动上传视频到公网 URL
- 容器名不对 → `docker ps` 查实际容器名，或手动传公网 URL
- 云 Tunnel 不通 → 检查 cloudflared 服务状态
...
python3 scripts/seedance.py create \
  ...
  --image char.png \
  --prompt "将视频中的角色替换成图片中的太空猫，动作和运镜不变" \
  --ratio 1:1 \
  --duration 5 \
  --wait
...
如果需要自己管理视频 URL，用 `--video-ref https://your-public-url/video.mp4` 直接传入。

### nginx 配置

img.aistar.work 已预配置静态视频路径，无需手动修改：
- `/video/*` → nginx 静态文件（绕过图床代理）
- `/*` → 图床代理（EasyImage 端口 40061）
...
| 模型 | Model ID | 能力 |
|------|----------|------|
| **Seedance 2.0 Fast**（脚本默认） | `doubao-seedance-2-0-fast-260128` | 文/图/视频/音频多模态、运动复刻、多镜头叙事，速度优先 |
...
# 视频参考 / 运动复刻 / 角色替换（2.0，自动上传本地视频到 nginx）
python3 scripts/seedance.py create --prompt "提示词" --video-ref motion_ref.mp4 --wait --download ~/Desktop

# 角色替换（图 + 视频参考）
python3 scripts/seedance.py create --image char.png --video-ref ref.mp4 --prompt "将视频中的角色替换成图片中的角色" --ratio 1:1 --wait --download ~/Desktop

# 音频参考 / 音乐卡点（2.0）
python3 scripts/seedance.py create --prompt "提示词" --audio bgm.mp3 --wait --download ~/Desktop

# 多模态混合（图+视频+音频，2.0）
python3 scripts/seedance.py create --prompt "提示词" --image img.jpg --video-ref ref.mp4 --audio bgm.mp3 --ratio adaptive --wait --download ~/Desktop
